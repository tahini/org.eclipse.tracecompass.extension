...
package org.eclipse.tracecompass.extension.internal.callstack.ui.handler;
    ...
    private ISymbolProviderPreferencePage[] getProviderPages() {
        ...
        if (trace != null) {
            for (ITmfTrace subTrace : TmfTraceManager.getTraceSet(trace)) {
                ISymbolProvider symbolProvider = SymbolProviderManager.getInstance().getSymbolProvider(subTrace);
                ISymbolProviderPreferencePage page = symbolProvider.createPreferencePage();
                if (page != null) {
                    pages.add(page);
                }
            }
